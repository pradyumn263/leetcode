...
public:
    ...
    bool areConnected(int x1, int x2) {
        int parentx1 = getRoot(x1);
        int parentx2 = getRoot(x2);

        return parentx1 == parentx2;
    }

    void unionify(int x1, int x2) {
        int parentx1 = getRoot(x1);
        int parentx2 = getRoot(x2);
        if(parentx2 != parentx1) {
    ...
    vector<int> findRedundantConnection(vector<vector<int>>& edges) {
        ...
        for(int i = 0; i < edges.size(); i++) {
            if(areConnected(edges[i][0], edges[i][1])) {
                res[0] = edges[i][0];
                res[1] = edges[i][1];
            } else {
                unionify(edges[i][0], edges[i][1]);
            }
        }
        return res;
    }
