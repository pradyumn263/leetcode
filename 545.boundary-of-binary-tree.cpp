...
public:
    ...
    void getBoundary(TreeNode* root, bool lb, bool rb) {
        if(!root)
            return;
        ...
        getBoundary(root->left, lb, !root->right && rb);
        getBoundary(root->right, !root->left && lb, rb);
    ...
    vector<int> boundaryOfBinaryTree(TreeNode* root) {
        ...
        boundary.push_back(root->val);
        getBoundary(root->left, true, false);
        getBoundary(root->right, false, true);
